...
$query_pag_data = "select * from buy_request where br_status = '1' and br_approval_status='0' and br_expired_time >='".date("Y-m-d")."' and br_u_id='".$_SESSION['uid']."' order by br_id desc LIMIT $start, $per_page";
$result_pag_data = mysql_query($query_pag_data) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from buy_request where br_status = '1' and br_approval_status='0' and br_expired_time >='".date("Y-m-d")."' and br_u_id='".$_SESSION['uid']."'"; // Total records
...
/*$sql = 'select * from buy_request where br_status = "1" and br_approval_status = "0" and br_u_id = "'.$_SESSION['uid'].'" order by br_id desc';
$res = mysql_query($sql);
...
<div class="dpl-tabs">
	<div class="dpl-pagenav dpl-pagenav-right dpl-tabs-right clearfix" id="matrixTabPage">
        ...
        <span class="next" onclick="javascript:pendingApprovalProdTabBR('<?php echo $nex; ?>')"></span>
        ...
        <?php	}	
			...
			if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
			<span class="prev" onclick="javascript:pendingApprovalProdTabBR('<?php echo $pre; ?>')"></span>
	...
	<ul class="clearfix" id="productTabs">
		...
		<li class="current" id="approvalPendingTab"><a href="javascript:pendingApprovalProdTabBR(1);"><span><?php echo $lang[481]; ?></span> (<span id="approvalPendingCount"><?php echo $count; ?></span>)</a></li>
...
<a class="stop-propagation" href="" title="dessx"><?php echo stripslashes($row->br_pd_name); ?></a>
<br>
  ...
  <div class="select" id="selectBottom">
	...
	// FOR ENABLING THE PREVIOUS BUTTON
		if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
            <a class="AE-paginator-page" href="javascript:pendingApprovalProdTabBR('<?php echo $pre; ?>')">&lt;&lt;<?php echo $lang[248]; ?></a>
	...
	<?php	} else	{	?>
			<a class="AE-paginator-page" href="javascript:pendingApprovalProdTabBR('<?php echo $i; ?>')"><?php echo $i; ?></a>
	<?php	}
		}
		...
		// TO ENABLE THE NEXT BUTTON	
			if ($next_btn && $cur_page < $no_of_paginations)
			{
				$nex = $cur_page + 1;
		?>
	        <a class="AE-paginator-page" href="javascript:pendingApprovalProdTabBR('<?php echo $nex; ?>')"><?php echo $lang[249]; ?>&gt;&gt;</a>
        <?php	}	else if ($next_btn)	{		?>
