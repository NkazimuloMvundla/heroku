...
$query_pag_data = "select * from city,country,product,user,measurement_unit,currency where ct_cn_id=cn_id and pd_fob_mu_id=mu_id and pd_cur_id=cur_id and u_ct_id=ct_id and pd_u_id=u_id and pd_status = '1' and pd_expiry_date >= '".date("Y-m-d")."' and pd_u_id = '".$_SESSION['uid']."' order by pd_id desc LIMIT $start, $per_page";
$result_pag_data = mysql_query($query_pag_data) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from city,country,product,user,measurement_unit,currency where ct_cn_id=cn_id and pd_fob_mu_id=mu_id and pd_cur_id=cur_id and u_ct_id=ct_id and pd_u_id=u_id and pd_status = '1' and pd_expiry_date >= '".date("Y-m-d")."' and pd_u_id = '".$_SESSION['uid']."'"; // Total records
...
$sql_Apprvd="SELECT count(*) AS count from city,country,product,user,measurement_unit,currency where ct_cn_id=cn_id and pd_fob_mu_id=mu_id and pd_cur_id=cur_id and u_ct_id=ct_id and pd_u_id=u_id and pd_status = '1' and pd_approval_status = '1' and pd_expiry_date >= '".date("Y-m-d")."' and pd_u_id = '".$_SESSION['uid']."'";
...
<div class="dpl-tabs">
        ...
        <span class="next" onclick="javascript:allProdTab('<?php echo $nex; ?>')"></span>
        ...
        <?php	}	
			...
			if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
			<span class="prev" onclick="javascript:allProdTab('<?php echo $pre; ?>')"></span>
		...
		<ul class="clearfix" id="productTabs">
			...
			<li id="approvedTab"><a onclick="javascript:approvedProdTab(1);"><span><?php echo $lang[480]; ?></span> (<span id="approvedCount"><?php echo $count_Apprvd['count']; ?></span>)</a></li>
			<li id="approvalPendingTab"><a onclick="javascript:pendingApprovalProdTab(1);"><span><?php echo $lang[481]; ?></span> (<span id="expiredCount"><?php echo $count_pendAppr['count']; ?></span>)</a></li>
...
<form name="mp_form" id="mp_form" method="post">
		<div style="margin-bottom: 0pt;" class="listBatch ifm us en-us ff3 approved basic" id="listBatchTop">
			...
			<!--<div class="group-menu-container ddm-more-ggs" id="matrixGGSMoreoperationTop"> <a href="javascript:;" class="group-menu-trigger">More</a></div>
			<div class="group-menu-container ddm-more" id="matrixMoreoperationTop"> <a href="javascript:;" class="group-menu-trigger">More</a></div>-->
		</div>
	</div>
...
<p><?php if($row->pd_approval_status == '1'){echo $lang[480];}else{ echo $lang[506];} ?></p>
<a title="<?php echo $lang[485]; ?>" style="display: inline;" href="javascript:showDisplyOpt(<?php echo $row->pd_id; ?>);" class="matrix-dropdownlist" id='displayBrd<?php echo $row->pd_id; ?>' width="100">
...
<a href="product-edit.php?pid=<?php echo md5($row->pd_id);?>" class="has-visited" target="_blank">Edit</a>
<a href="javascript:arwDiv(<?php echo $row->pd_id; ?>);" class="matrix-dropdownlist matrix-dropdownlist-arrow">&nbsp;</a>

<ul style="display: none;" id="arrow<?php echo $row->pd_id; ?>" class="matrix-dropdownlist-panel matrix-dropdownlist-arrow-panel matrix-dropdownlist-show">
