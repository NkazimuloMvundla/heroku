...
$query_pag_data = "select * from buy_request where br_status = '1' and br_approval_status='1' and br_expired_time >='".date("Y-m-d")."' and br_u_id='".$_SESSION['uid']."' order by br_id desc LIMIT $start, $per_page";
$result_pag_data = mysql_query($query_pag_data) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from buy_request where br_status = '1' and br_approval_status='1' and br_expired_time >='".date("Y-m-d")."' and br_u_id='".$_SESSION['uid']."'"; // Total records
...
/*************************************************/
/*$sql = 'select * from buy_request where br_status = "1" and br_approval_status = "1" and br_u_id = "'.$_SESSION['uid'].'" order by br_id desc';
$res = mysql_query($sql);
...
$sql_all="SELECT count(*) AS count from buy_request where br_status = '1' and br_expired_time >='".date("Y-m-d")."' and br_u_id = '".$_SESSION['uid']."'";
...
<div class="dpl-tabs">
	<div class="dpl-pagenav dpl-pagenav-right dpl-tabs-right clearfix" id="matrixTabPage">
        ...
        <span class="next" onclick="javascript:approvedProdTabBR('<?php echo $nex; ?>')"></span>
        ...
        <?php	}	
			...
			if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
			<span class="prev" onclick="javascript:approvedProdTabBR('<?php echo $pre; ?>')"></span>
	...
	<ul class="clearfix" id="productTabs">
		<li id="allTab"><a href="javascript:allProdTabBR(1);"><span><?php echo $lang[428]; ?></span> (<span id="allCount"><?php echo $count_all['count']; ?></span>)</a></li>
		<li class="current" id="approvedTab"><a href="javascript:approvedProdTabBR(1);"><span><?php echo $lang[480]; ?></span> (<span id="approvedCount"><?php echo $count; ?></span>)</a></li>
...
<a class="stop-propagation" href=""><?php echo stripslashes($row->br_pd_name); ?></a>
<br>
...
<a style="display: inline;" href="javascript:showDisplyOpt(<?php echo $row->br_id; ?>);" class="matrix-dropdownlist" id='displayBrd<?php echo $row->br_id; ?>' width="100">
...
Edit</a>
<a href="javascript:;" onclick="javascript:arwDiv(<?php echo $row->br_id; ?>);" class="matrix-dropdownlist matrix-dropdownlist-arrow">&nbsp;</a>

<ul id="arrow<?php echo $row->br_id; ?>" class="matrix-dropdownlist-panel matrix-dropdownlist-arrow-panel matrix-dropdownlist-show">
  ...
  <div class="select" id="selectBottom">
	...
	// FOR ENABLING THE PREVIOUS BUTTON
		if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
            <a class="AE-paginator-page" href="javascript:approvedProdTabBR('<?php echo $pre; ?>')">&lt;&lt;<?php echo $lang[248]; ?></a>
	...
	<?php	} else	{	?>
			<a class="AE-paginator-page" href="javascript:approvedProdTabBR('<?php echo $i; ?>')"><?php echo $i; ?></a>
	<?php	}
		}
		...
		// TO ENABLE THE NEXT BUTTON	
			if ($next_btn && $cur_page < $no_of_paginations)
			{
				$nex = $cur_page + 1;
		?>
	        <a class="AE-paginator-page" href="javascript:approvedProdTabBR('<?php echo $nex; ?>')"><?php echo $lang[249]; ?>&gt;&gt;</a>
        <?php	}	else if ($next_btn)	{		?>
