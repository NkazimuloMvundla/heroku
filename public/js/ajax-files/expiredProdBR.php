...
$query_pag_data = "select * from buy_request where br_status = '1' and br_u_id = '".$_SESSION['uid']."' and br_expired_time < '".date("Y-m-d")."' order by br_id desc LIMIT $start, $per_page";
...
<div class="dpl-tabs">
	...
	<ul class="clearfix" id="productTabs">
		...
		<li id="approvalPendingTab"><a href="javascript:pendingApprovalProdTabBR(1);"><span><?php echo $lang[481]; ?></span> (<span id="approvalPendingCount"><?php echo $count_pendAppr['count']; ?></span>)</a></li>
            <li class="current" id="expiredTab"><a href="javascript:expiredProdTabBR(1);"><span><?php echo $lang[48]; ?></span> (<span id="allCount"><?php echo $count; ?></span>)</a></li>
	</ul>
</div>
  ...
  <div class="select" id="selectBottom">
	...
	// FOR ENABLING THE PREVIOUS BUTTON
		if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
            <a class="AE-paginator-page" href="javascript:expiredProdTabBR('<?php echo $pre; ?>')">&lt;&lt;<?php echo $lang[248]; ?></a>
	...
	<?php	} else	{	?>
			<a class="AE-paginator-page" href="javascript:expiredProdTabBR('<?php echo $i; ?>')"><?php echo $i; ?></a>
	<?php	}
		}
		...
		// TO ENABLE THE NEXT BUTTON	
			if ($next_btn && $cur_page < $no_of_paginations)
			{
				$nex = $cur_page + 1;
		?>
	        <a class="AE-paginator-page" href="javascript:expiredProdTabBR('<?php echo $nex; ?>')"><?php echo $lang[249]; ?>&gt;&gt;</a>
        <?php	}	else if ($next_btn)	{		?>
