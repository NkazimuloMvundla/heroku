...
$query_pag_data = "select * from city,country,product,user,measurement_unit,currency where ct_cn_id=cn_id and pd_fob_mu_id=mu_id and pd_cur_id=cur_id and u_ct_id=ct_id and pd_u_id=u_id and pd_status = '1' and pd_approval_status = '0' and pd_u_id = '".$_SESSION['uid']."' order by pd_id desc LIMIT $start, $per_page";
$result_pag_data = mysql_query($query_pag_data) or die('MySql Error' . mysql_error());

/* -----Total count--- */
$query_pag_num = "SELECT count(*) AS count from city,country,product,user,measurement_unit,currency where ct_cn_id=cn_id and pd_fob_mu_id=mu_id and pd_cur_id=cur_id and u_ct_id=ct_id and pd_u_id=u_id and pd_status = '1' and pd_approval_status = '0' and pd_u_id = '".$_SESSION['uid']."'"; // Total records
...
/*************************************************************/
//$sql = 'select * from product where pd_status = "1" and pd_approval_status = "0" and pd_u_id = "'.$_SESSION['uid'].'" order by pd_id desc';
//$res = mysql_query($sql);
	...
	<div class="dpl-pagenav dpl-pagenav-right dpl-tabs-right clearfix" id="matrixTabPage">
		...
		// TO ENABLE THE NEXT BUTTON	
			if ($next_btn && $cur_page < $no_of_paginations)
			{
				$nex = $cur_page + 1;
		?>
        <span class="next" onclick="javascript:pendingApprovalProdTab('<?php echo $nex; ?>')"></span>
        ...
        <?php	}	
			...
			if($previous_btn && $cur_page > 1){
				$pre = $cur_page - 1;
		?>
			<span class="prev" onclick="javascript:pendingApprovalProdTab('<?php echo $pre; ?>')"></span>
		...
		<ul class="clearfix" id="productTabs">
			...
			<li class="current" id="approvalPendingTab"><a onclick="javascript:pendingApprovalProdTab(1);"><span><?php echo $lang[481]; ?></span> (<span id="approvalPendingCount"><?php echo $count; ?></span>)</a></li>
...
<td id="dataTableApproved_0_2" class="matrix-column-subject AE-datatable-row-0 AE-datatable-col-2" width="320"><?php echo stripslashes($row->pd_name); ?><br><span class="matrix-redModel"></span><br></td>
...
<td id="dataTableApproved_0_4" class="status AE-datatable-row-0 AE-datatable-col-4" width="115" style="display:none">
